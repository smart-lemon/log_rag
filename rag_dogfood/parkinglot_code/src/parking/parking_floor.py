...
from src.parking.parkingdisplay import *
...
class ParkingFloor:
    ...
    def free_spot(self, spot):
        ...
        type = spot.get_type()
        if type == ParkingSpotType.HANDICAPPED: self.__free_handicapped_spot_count += 1
        if type == ParkingSpotType.COMPACT: self.__free_compact_spot_count += 1
        if type == ParkingSpotType.LARGE: self.__free_large_spot_count += 1
        if type == ParkingSpotType.MOTORBIKE: self.__free_motorbike_spot_count += 1
        if type == ParkingSpotType.ELECTRIC: self.__free_electric_spot_count += 1
